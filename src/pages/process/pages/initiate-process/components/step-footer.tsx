...
const StepFooter: React.FC = () => {
  ...
  const submitProcess = async () => {
    try {
      const initiateValue = getFormInstance('initiate').getFieldsValue(true);
      const uploadValue = getFormInstance('upload').getFieldsValue(true);
      console.log(initiateValue, uploadValue);
      setLoading(true);
    } catch (error) {
      message.error("提交失败");
    } finally {
  ...
  return (
    ...
    >
      ...
      {
        stepIndex === 2 && (
          <Button type="primary" loading={loading} onClick={submitProcess} disabled={loading} >
            {timeOut <= 0 ? '' : `${timeOut} s`}
            我已知晓，确认创建
          </Button>
        )
